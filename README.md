...
Планировщик запущен в отдельном контейнере `celery-beat` и генерирует события строго по расписанию.

### Расписание:

| Задача                  | Расписание                    | Описание                                          |
|:------------------------|:------------------------------|:--------------------------------------------------|
| `cleanup_expired_tasks` | Каждые 5 минут (`300s`)       | Демонстрационный прогон очистки устаревших данных |
| `cleanup_expired_tasks` | Ежедневно в 03:00 (`crontab`) | Удаление завершенных задач старше 30 дней         |

### Мониторинг логов:

```bash
# Логи отправки задач планировщиком
docker compose logs -f celery-beat

# Логи выполнения задач воркером
docker compose logs -f celery
```